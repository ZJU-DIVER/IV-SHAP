...
def ccshap_confidence(model_name, input_data, mean_data, input_seperate, initial_m, local_m) -> np.ndarray:
    ...
    for _ in range(local_m):
        ...
        for j in range(math.ceil(n / 2) - 1, n):
            for i in range(n):
                sigma_j[j] += var[i][j] / count[i][j]
                if n - j - 2 < 0:
                    sigma_n_j[j] += 0
                else:
                    sigma_n_j[j] += var[i][n - j - 2] / count[i][n-j-2]
        
        stra = math.ceil(n / 2) - 1
        interval_distance = 0 
        for j in range(math.ceil(n / 2) - 1, n):
            if interval_distance < sigma_j[j] + sigma_n_j[j]:
                interval_distance = sigma_j[j] + sigma_n_j[j]
                stra = j
        j = stra
        cals[j] += 1
        idxs = np.arange(n)
